  ...
  return (
    <Box p={5}>
      ...
      <Box mb={4}>
        ...
        <Button leftIcon={<FaPlus />} ml={2} colorScheme="teal" onClick={handleAddTodo}>
          Add
        </Button>
      </Box>
      <List spacing={3}>
        {todos.map((todo, index) => (
          <ListItem key={index} d="flex" alignItems="center">
            {todo}
            <IconButton icon={<FaTrash />} colorScheme="red" variant="ghost" ml="auto" onClick={() => handleDeleteTodo(index)} />
          </ListItem>
        ))}
      </List>
    </Box>
